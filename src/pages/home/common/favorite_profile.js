...
function Favorite_profile(props) {
    const navigate = useNavigate();
    const [remove, setremove] = useState(false)
    const [removecheck, setremovecheck] = useState(props.heart || props.heart2 == true ? true : false)
    ...
    const favourite_selectapi = (a) => {
        ...
        fetch(api + "/api/v1/savefavprofile", requestOptions)
            .then(response => response.json())
            .then(result => {
                props.heart ? props.heart() :
                    console.log(result)
            })
                        ...
                        <Modal show={remove} onHide={e => setremove(false)} >
                            <Modal.Body>
                                <div className='promocode_content signout invite favoritemod'>
                                    ...
                                    <p>Are you sure you want to remove  <span style={{ color: "#A98D4B" }}>{props.username} </span>from Favorites? </p>
                                    ...
                                    <p class="hidemod">
                                       Please visit our <Link to=" /faq " target="_blank" > FAQ </Link> and  <Link to="/safety-center" target="_blank">Safety center</Link> to read more about SensCare platform and our safety tips.
                                    </p>
                                </div>
                            </Modal.Body>
                        </Modal >
...
export default Favorite_profile
