...
function Faq() {
    ...
    const [search, setsearch] = useState(localStorage.getItem("faq") ? localStorage.getItem("faq") : "")
                ...
                <div className='container'>
                    ...
                    <div className='form_group'>
                        <form name="myForm" action="javascript:myFunction(); return false;">
                            <div className='inp'>
                                <input placeholder="Enter keyword" type={"text"} value={search} onChange={e => {
                                    setsearch(e.target.value)
                                    localStorage.removeItem("faq")
                                }} />
                                {search.length > 1 ? <span onClick={e => searchclear()}>+</span> : ""}
                            </div>
                            <button type='submit' onClick={e => search_fi()}><img src='./images/search_res.svg' />
                                Search </button>
                        </form>
                    </div>
                </div>
                ...
                <div className='container'>
                    <div className='resource_content'>
                        <div className='resources_tabs faq'>
                            {content.length >= 1 ? content.map((data, index2) => {
                                if (data.data.length != 0) {
                                    if (usertype == "provider") {
                                            ...
                                            <div className='tabs'>
                                                {
                                                    record != "" ? <h2 onClick={e => {
                                                        ...
                                                        localStorage.removeItem("faq")
                                                        setcount(true)
                                                    }} >{data.main_category}</h2> : <h2>{data.main_category} </h2>}
                                                <div className='tab_open' >
                                                    {data.data.map((data2, index) => {
                                                        return (
                                                            <>
                                                                <div className='faqans'>
                                                                    {(data.id == 17 ? data2.category && index == 0 || index == 4 || index == 16 || index == 30 || index == 32 || index == 36 : data.id == 58 ? data2.category && index == 0 || index == 5 || index == 13 : data2.category) ? <h3>{data2.category} </h3> : ""}
                                                                    {(data.id == 17 ? data2.sub_category && index == 4 || index == 13 || index == 16 || index == 26 : data2.sub_category) ? <h5>{data2.sub_category} </h5> : ""}
                                                                    <h4 id={"tag" + index2 + index} onClick={e => filterarray("tab" + index2 + index, "icon" + index2 + index, "tag" + index2 + index)}><span>{data2.question} </span> <img src='./images/done_a.svg' id={"icon" + index2 + index} /></h4>
                                                                    <div dangerouslySetInnerHTML={{ __html: data2.answer }} className={"workus" + " " + (record != "" ? "answeropn" : "answer")} id={"tab" + index2 + index} />
                                                                    <div />
                                                                </div>
                                        ...
                                        if (data.main_category != "For Business and schools" && data.main_category != "EDU Store") {
                                            return (
                                                <div className='tabs'>
                                                    {
                                                        record != "" ? <h2 onClick={e => {
