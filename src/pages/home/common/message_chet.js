            ...
            {list && list[0] ?
                <>
                    ...
                    <div className='message_chet' >
                            ...
                            list && list[0].userdetails ?
                                <div className='top_profile' >
                                    <div className='left_p' onClick={e => setactive(false)}>
                                        <img src={api + "/public/assets/images/users/" + (list && list[0].userdetails?.file_path)} />
                                        <span>
                                            <h2>{list && list[0].userdetails?.first_name} {list && list[0].userdetails?.last_name}</h2>
                                            <p>
                                                {list && list[0].userdetails?.reviewAvg >= 0 ?
                                                    <>
                                                        {[...Array(list && list[0].userdetails?.reviewAvg)].map((star, index) => {
                                                            index += 1;
                                                            return (
                                                        ...
                                                        {[...Array(5 - (list && list[0].userdetails?.reviewAvg))].map((star, index) => {
                                                            index += 1;
                                                            return (
                                    ...
                                    {
                                        active ?
                                            <div className='bg_off' onClick={e => setactive(false)}></div>
                                            : ""
                                    }

                                    <div className='right_p'>
                                        <Link to={list && list[0] && list[0]?.userdetails?.user_type == "provider" ? "/profile-provider/" + (list && list[0] && list[0]?.userdetails?.id) : "/profile-parents/" + (list && list[0] && list[0]?.userdetails?.id)}>
                                           View profile
                                        </Link>
                        ...
                        <div className='chet_message' id='parentDiv' onClick={e => setactive(false)}>
                            {
                                list && list[0].getmessages ?
                                    list && list[0].getmessages.map((data) => {
                                        return (
                                            <div className={data.sender_id == plink ? 'send' : 'recive'}>
                                                <img src={api + "/public/assets/images/users/" + (data.sender_id == plink ? data.senderImage : list && list[0].userdetails?.file_path)} />
                                                <div className='message_view'>
                                                    {data.message}
                                                </div>
                                                <span>
                                                    {data.created_at ? new Date(data.created_at).toLocaleDateString('en-US', { weekday: 'short' }) : ""} , {data.created_at ? month[new Date(data.created_at).getMonth()] : ""}, {new Date(data.created_at).getDate()}, {new Date(data.created_at).getFullYear()}, {new Date(data.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                </span>
                                            </div>
                                        )
                                    })
                            ...
                            list && list[0].userdetails?.blockUser == false ?
                                list && list[0]?.userdetails?.blockbycurrentuser == false ?
                                    <div className='send_input'>
                                        ...
                                        <textarea placeholder='Type here' type="textarea" id='textarea' value={message} onChange={e => setmessage(e.target.value)} />
                                        <button onClick={accept} style={messagecheck == true ? { opacity: "0.5" } : {}} disabled={messagecheck}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="27" viewBox="0 0 16 17" fill="none">
                        ...
                        {list && list[0].userdetails?.blockUser == true ? <p className='blocked' onClick={block}>
                            Blocked</p> : list && list[0]?.userdetails?.blockbycurrentuser == true ? <p className='blocked' >
                                This user blocked you.</p> : ""}
                    </div>
                            ...
                            <div className='promocode_content cancelmembership report'>
                                ...
                                <div className='cancelmembershipp'>
                                    ...
                                    <div className='comment'>
                                        <textarea className={report.resion == "other" && report.other == "" ? "bordererror" : ""} maxlength="70" placeholder="Type here..." onChange={e => setreport({ ...report, other: e.target.value, error: "" })}></textarea>
                                        <span >Number of characters  {70 - report.other.length}</span>
                                    </div>
                                    <div class="button text-center">
                                        <div class="pull-right">
                                            <button class="btn" onClick={e => repost()}>Report User</button>
                                        </div>
                                        <div class="pull-right">
                                            <button class="btn confirm" onClick={e => setsurvay(false)}>Cancel</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <Modal.Body>
                            <div className='promocode_content cancelmembership reportthanks'>
                                <div className=''>
                                    ...
                                    <br />
                                    <p className='giveus'>In the meantime, please visit our <Link to="/safety-center" target={'_blank'}>Safety center</Link>and <Link to="/faq" target={'_blank'}>FAQ</Link> for more info.</p>
