...
function Recommendations_parents_profile(props) {
    ...
    const recommendation = () => {
        ...
        fetch(api + "/api/v1/parentsrecommendationrequestresponsesend", requestOptions)
            .then(response => response.json())
            .then(result => {
                setdocmodel(false)
                setdoc("")
                ...
                <>
                    ...
                    <div className='detail_invit'>
                        <p style={{ fontSize: "16px", fontWeight: "600" }}>
                            <Link to={"/profile-provider/" + list[0]?.user_id} >{list && list[0] && list[0].ProviderName}</Link> requested a recommendation from you  <span className='date'>{list && list[0] && list[0].created_at ? new Date(list && list[0] && list[0].created_at).toLocaleDateString('en-US', { weekday: 'short' }) : ""} , {list && list[0] && list[0].created_at ? month[new Date(list && list[0] && list[0].created_at).getMonth()] : ""}, {new Date(list && list[0] && list[0].created_at).getDate()}, {new Date(list && list[0] && list[0].created_at).getFullYear()}, {new Date(list && list[0] && list[0].created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></p>
                        <h2 style={{ padding: "15px 0 0" }}>Dear <span style={{ textTransform: "capitalize", marginLeft: "0" }}><span>{message}</span></span>,
                        </h2>
                        <h2>
                       You’ve been requested to give a recommendation!
                        </h2>
                        <p>A recommendation will increase user's chances of getting hired.</p>
                    </div>
                                ...
                                list.map((data, index) => {
                                    return (
                                        <div className='recommend recomendbig'>
                                            <div className='image'>
                                                <img src={data.ProviderImage != null ? api + "/public/assets/images/users/" + data.ProviderImage : window.location.origin + window.location.origin + "/img/nany_img.png"} alt="" />
                                            </div>
                                            <div className='detail'>
                                                <h3>{data.ProviderName}</h3>
                                                <p>{data.recommendation_message}</p>
                                                <button disabled={data.recommendation_response != null || recommend != "" ? true : false} onClick={e => setdocmodel(true)} style={data.recommendation_response != null || recommend != "" ? { opacity: "0.5" } : {}} >Leave Recommendation</button>
                                            </div>
                                            <p>The recommendation letter should contain your feedback regarding providers' work achievements and responsibilities.
                                                It also can help other families to verify future candidate eligibility and performances</p>
                                        </div>
                                    )
                                }
                                )}
                ...
                <Modal show={docmodel} onHide={e => setdocmodel(false)
                    ...
                    <Modal.Body>
                        <div className='promocode_content signout senddoc'>
                            <Link to="" onClick={e => setdocmodel(false)}>+ </Link>
                            ...
                            <ul>
                                ...
                                <li>
                                    <span class="view-message  dont-show">
                                        <input type="radio" class="mail-radio" name="chk" defaultChecked="true" />
                                        <span className='circle'></span>
                                    </span>
                                    <span className='bothtype'>
                            ...
                            <div className='file'>
                                ...
                                <div className='group_label'>
                                    <span>{doc == "" ? "Choose file" : doc.name}</span>
