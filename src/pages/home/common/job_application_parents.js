...
import Job_application_detail from './job_application_detail';
import { Link, useNavigate, useParams } from 'react-router-dom'


export default function Job_application_hiatory_parents() {
    ...
    const list = () => {
        ...
        fetch(api + "/api/v1/jobapplicationsnotification?" + (refine.threedays != "" ? "threedays=" + refine.threedays : refine.withinweek != "" ? "withinweek=" + refine.withinweek : refine.twofourday != "" ? "twofourdays=" + refine.twofourday : refine.withinmonth != "" ? "withinmonth=" + refine.withinmonth : ""), requestOptions)
            .then(response => response.json())
            .then(result => setinterview(result.data.notifications))
            ...
            {view == "" ?
                <>
                    <div class="main-header">
                        <h2 className='border'>Job Applications</h2>
                        <div class="mail-header-bar">
                            <p>{interview.length} Job Applications </p>
                        ...
                        <tbody>
                            {
                                interview.map((data) => {
                                    return (
                                        <tr class={data.jobStatus != 1 ? data.read_status == 0 || (new Date(data.created_at).getDate() > new Date().getDate() ?
                                            new Date(data.created_at).getMonth() > new Date().getMonth() :
                                            new Date(data.created_at).getDate() >= new Date().getDate() - 7) ? "unread" : data.status == 0 ? "unread2" : "close" : "close"} >
                                            <td class="view-message  dont-show">
                                                ...
                                                <span onClick={e => {
                                                    ...
                                                    setview(data.id)
                                                        ...
                                                        data.jobStatus != 1 ?
                                                            data.read_status == 0 || (new Date(data.created_at).getDate() > new Date().getDate() ?
                                                                new Date(data.created_at).getMonth() > new Date().getMonth() :
                                                    ...
                                                    <b onClick={e => {
                                                        navigate("/search-parents/" + slugdata.id + "/" + data.id)
                                                        setview(data.id)
                                                    }}>Congratulations,  <span class="username-text" > {data.Appliedusername} </span> applied to your job post </b> <p> - {data.jobTitle}
                                                    </p>
                                                </span>
                                            </td>
                ...
                <div class="main-header">
                    ...
                    <Job_application_detail id={view} />
                </div>
            }
        </div>
    )
