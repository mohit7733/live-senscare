...
import Hires_detail from './hires_detail';
...
export default function Hired() {
    ...
    const list = () => {
        ...
        fetch(api + "/api/v1/hiring_notifications?" + (refine.threedays != "" ? "threedays=" + refine.threedays : refine.withinweek != "" ? "withinweek=" + refine.withinweek : refine.twofourday != "" ? "twofourday=" + refine.twofourday : refine.withinmonth != "" ? "withinmonth=" + refine.withinmonth : ""), requestOptions)
            .then(response => response.json())
            .then(result => setinterview(result.data.allnotificatons))
    ...
    const delete_in = () => {
        ...
        fetch(api + "/api/v1/hiring_notificationsdelete", requestOptions)
            ...
            {view == "" ?
                <>
                    <div class="main-header">
                        <h2 className='border'>New Hirings</h2>
                        <div class="mail-header-bar">
                            <p>{interview.length} New Hirings </p>
                    ...
                    <table class="table table-inbox table-hover hire">
                        <tbody>
                            {
                                interview.map((data) => {
                                    return (
                                        <tr class={data.parent_readstatus == 0 ? "unread" : ""} >
                                            <td class="view-message  dont-show">
                                                ...
                                                <span onClick={e => {
                                                    ...
                                                    Congratulations {window.innerWidth < 720 ? "!" : ","}<br /><span class="username-text" onClick={e => {
                                                        navigate("/search-providers/" + slugdata.id + "/" + data.id)
                                                        setview(data.id)
                                                    }}>{data.ParentsName} </span>has just hired you.
                                                </span>
                                            </td>
