...
function Resources() {
    ...
    const search_fi = () => {
        ...
        fetch(api + "/api/resourcecategory?category=" + (selectcat.map((e) => {
            return e.name
        })) + "&anysearch=" + keysearch, requestOptions2)
            .then(response => response.json())
            .then(result => {
            ...
            <div className='container-fluid resource_tital'>
                <div className='container'>
                        ...
                        <h2>{investor.title} </h2> : ""}
                    {
                        ...
                        <div className="workus" dangerouslySetInnerHTML={{ __html: investor.description }} /> : ""
                    }
                </div>
            </div>
                ...
                <div className='container'>
                    <div className='resource_content'>
                        ...
                        <div className='resource_search'>
                            <label>Explore resources <Link to="" onClick={e => {
                                window.location.reload()
                            }}>Clear all filters</Link></label>
                            <div className='form_group'>
                                <div className='inp'>
                                    <input placeholder="keyword" className='keyword' type="text" onChange={e => setkeysearch(e.target.value)} value={keysearch} />
                                    {keysearch.length > 2 ? <span onClick={e => {
                                        setkeysearch("")
                                        setcategory("")
                                        setTimeout(() => {
                                            search_fi()
                                ...
                                <div className='customselect inp'>
                                    <input className='keyword' type="text" placeholder='Category' value={selectcat.map((e) => {
                                        return e.name
                                    })} />
                                    <div className={`option ${catopen ? 'open' : ''}`} id='cate' style={{ display: display }} >

                                        {
                                            searchselect.map((e, index) => {
                                                return <p className={searchselect.length == (1 + index) ? 'border' : ''} ><input type="checkbox" onClick={a => selectoption(e.category)} /><h3>{e.category}  </h3><span></span></p>
                                            })
                                        }
                                    ...
                                    <span onClick={e => custom("cate")} id='svgIcon'>
                            ...
                            <div className='clear'>
                                ...
                                <button onClick={e => {
                                    ...
                                    <svg width="20" height="21" viewBox="0 0 20 21" fill="" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <path fill-rule="evenodd" clip-rule="evenodd" d="M13.9433 14.1498C14.3338 13.7593 14.967 13.7593 15.3575 14.1498L19.7075 18.4999C20.0981 18.8904 20.0981 19.5235 19.7075 19.9141C19.317 20.3046 18.6838 20.3046 18.2933 19.9141L13.9433 15.5641C13.5528 15.1735 13.5528 14.5404 13.9433 14.1498Z" />
                                    </svg>
                                   Search </button>
                            </div>
                        </div>
                        <div className='resources_tabs'>
                            {content.length >= 1 ? content.map((data, index) => {
                                return (
                                    <div className='tabs'>
                                        <h2 onClick={e => filterarray("tab" + index, "icon" + index)}>
                                            {data.category}  <img src='./images/down_pur.svg' id={"icon" + index} /></h2>
                                        <div className='tab_open' id={"tab" + index}>
                                            {data.data.map((data2, index) => {
                                                return (
                                                    <>
                                                        {data2.title ?
                                                            <h3>{data2.title} </h3> : <br />}
                                                        {data2.subtitle ?
                                ...
                                : <p>No records matching the current criteria were found!</p>}
                        </div>
