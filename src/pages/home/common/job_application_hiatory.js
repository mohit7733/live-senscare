...
import Job_application_hiatory_detail from './job_application_hiatory_detail';
import { useParams, useNavigate } from 'react-router-dom';



export default function Job_application_hiatory() {
    ...
    const list = (a) => {
        ...
        fetch(api + "/api/v1/providerappliedjobslist?" + (a != undefined ? a == "New" ? "new_search=" + 0 : "job_search=" + a : refine), requestOptions)
            .then(response => response.json())
            .then(result => setinterview(result.data.providerappliedjobslist))
        ...
        <div>
            {view == "" ?
                            ...
                            <div class="btn-group flex">
                             Refine by
                                <div className='select'>
                                    <label onClick={e => setactive(!active)}>{refine} <span><img src="/images/done_a.svg" /></span></label>
                                    {
                                        active ?
                        ...
                        <tbody>
                            {
                                interview.map((data) => {
                                    return (
                                        <tr class={data.jobStatus == 0 ? data.provider_readstatus == 0 || (
                                            new Date(data.created_at).getDate() > new Date().getDate() ?
                                                new Date(data.created_at).getMonth() > new Date().getMonth() :
                                                new Date(data.created_at).getDate() >= new Date().getDate() - 7) ? "unread" : data.jobStatus == 0 ? "unread2" : "close" : "close"} >
                                                    ...
                                                    setview(data.id)
                                                        ...
                                                        data.jobStatus == 0 ?
                                                            data.provider_readstatus == 0 || (
                                                                ...
                                                                : data.jobStatus == 0 ? <span class="new-tag">Active </span> : data.jobStatus >= 0 ? <span class="new-tag">Closed </span> : ""
                                                            : <span class="new-tag"> Closed</span>
                                                    }
                                                    <strong onClick={e => {
                                                        navigate("/search-providers/" + slugdata.id + "/" + data.id)
                                                        setview(data.id)
                                                    }}>{data.title.substr(0, 40)}</strong> - {data.description.substr(0, 40)}
                                                </span>
                                            </td>
                ...
                <div class="main-header">
                    ...
                    <Job_application_hiatory_detail id={view} />
                </div>
            }
        </div>
    )
