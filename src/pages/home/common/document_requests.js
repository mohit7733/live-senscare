...
import Document_requests_main_detail from './document_requests_main_detail';
import Invite_detail from './invite_detail';
import { useParams, useNavigate } from 'react-router-dom';


export default function Document_requests() {
    ...
    const list = () => {
        ...
        fetch(api + "/api/v1/documentrequest_notifications?" + (refine.threedays != "" ? "threedays=" + refine.threedays : refine.withinweek != "" ? "withinweek=" + refine.withinweek : refine.twofourday != "" ? "twofourday=" + refine.twofourday : refine.withinmonth != "" ? "withinmonth=" + refine.withinmonth : ""), requestOptions)
    ...
    const delete_in = () => {
        ...
        fetch(api + "/api/v1/multideletedocumentrequest", requestOptions)
            ...
            {view == "" ?
                <>
                    <div class="main-header">
                        <h2 className='border'>Document Requests</h2>
                        <div class="mail-header-bar">
                            <p>{interview.length} Requests </p>
                        ...
                        <tbody>
                            {
                                interview.map((data) => {
                                    return (
                                        <tr class={data.documentrequest_readstatus == 0 ? "unread" : ""} >
                                            <td class="view-message  dont-show">
                                                ...
                                                <span onClick={e => {
                                                    ...
                                                    setview(data.id)
                                                        ...
                                                        data.documentrequest_readstatus == 0 ?
                                                    ...
                                                    <span class="username-text" onClick={e => {
                                                        navigate("/search-providers/" + slugdata.id + "/" + data.id)
                                                        setview(data.id)
                                                    }}> {data.ParentsName}</span> has just requested  <strong>{data?.documment_name.substr(data?.documment_name.lastIndexOf('\\') + 1).split('[')[1] ? JSON.parse(data?.documment_name).map((e, i) => e + ((i + 1) == JSON.parse(data?.documment_name).length ? "" : ", ")) : data?.documment_name}</strong>
                                                </span>
                                            </td>
                ...
                <div class="main-header">
                    ...
                    <Document_requests_main_detail id={view} />
                </div>
            }
        </div>
    )
