...
import Interview_accept from './common/interview_accept_detail';
import { Link, useNavigate, useParams } from 'react-router-dom'


export default function Interview_invite_job() {
    ...
    const list = () => {
        ...
        fetch(api + "/api/v1/parentsinterviewinvites_notification?" + (refine.threedays != "" ? "threedays=" + refine.threedays : refine.withinweek != "" ? "withinweek=" + refine.withinweek : refine.twofourday != "" ? "twofourday=" + refine.twofourday : refine.withinmonth != "" ? "withinmonth=" + refine.withinmonth : ""), requestOptions)
    ...
    const delete_in = () => {
        ...
        fetch(api + "/api/v1/multideleteparentsinvitaion", requestOptions)
                        ...
                        <tbody>
                            {
                                interview.map((data) => {
                                    return (
                                        <tr class={data.jobStatus == 0 ? data.parent_readstatus == 0 ? "unread" : "" : "close"} >
                                                    ...
                                                    setview(data.id)
                                                        ...
                                                        data.jobStatus == 0 ?
                                                            data.parent_readstatus == 0 ?
                                                                ...
                                                                : ""
                                                            : <span class="new-tag">Closed</span>
                                                    }
                                                    {
                                                        <>
                                                            <b>{data.status == 1 ? "Congratulations"  : "We're sorry" }, <span class={data.status == 1 ? "username-text" : ""} style={data.status == 1 ? {} : { fontWeight: "500" }} onClick={e => {
                                                                navigate("/search-parents/" + slugdata.id + "/" + data.id)
                                                                setview(data.id)
                                                            }}> {data.ProviderName} </span> {data.status == 1 ? "accepted "  : "Declined " } your Interview invite</b>
                                                            <p> - {data.title != null ? (data.title.substr(0, 50)
                                                            ) : ""}</p>
                                                        </>

                                                    }</span>
                                            </td>
                                            <td class="view-message  text-right">{month[new Date(data.invite_responsedate).getMonth()] + " " + (parseInt(new Date(data.invite_responsedate).getDate()) < 10 ? "0" : "") + new Date(data.invite_responsedate).getDate()}</td>
                                        </tr>
                                    )
                                })
                            }
